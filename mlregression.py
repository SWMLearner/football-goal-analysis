...
Original file is located at
    https://colab.research.google.com/drive/1ZfcRmrEKabmQ4YlFhFZndwADmZeTyTGo
"""
...
# Load data
df = pd.read_csv('/kaggle/input/football-coaching/football.csv')
# Set up plotting
plt.style.use('seaborn-v0_8')
sns.set_palette("husl")

print("📊 FOOTBALL GOAL ANALYSIS: EXPLANATORY MODELING")
print("=" * 60)
...
print("\n🔍 EXPLANATORY vs PREDICTIVE MODELING CLARIFICATION")
print("-" * 50)
...
if high_corr_pairs:
    ...
    print("✅ No high correlations detected - features are relatively independent")
...
print("⚠️  VIF 5-10: Moderate multicollinearity")
print("❌ VIF > 10: High multicollinearity")

# Check for any high VIF issues
high_vif = vif_data[vif_data['VIF'] > 5]
if not high_vif.empty:
    print(f"\n⚠️  Features with moderate/high VIF:")
    for _, row in high_vif.iterrows():
        print(f"   • {row['Feature']}: VIF = {row['VIF']:.2f}")
else:
    print("✅ All features have low multicollinearity (VIF < 5)")
...
print("🔍 MODELING APPROACH: EXPLANATORY (using entire dataset)")
...
print(f"\n🏆 BEST MODEL FOR EXPLANATORY ANALYSIS: {best_model_name}")
...
print(f"   • Home teams average: {home_avg:.2f} goals per match")
print(f"   • Away teams average: {away_avg:.2f} goals per match")
print(f"   • Home advantage: {home_advantage:.2f} goals per match")
print(f"   • Home teams score {home_avg/away_avg:.1%} more than away teams")

print(f"\n🎯 MODEL PERFORMANCE INSIGHTS")
print(f"   • Best model ({best_model_name}) explains {best_model_stats['Adjusted_R2']:.1%} of goal variance")
print(f"   • Cross-validation shows consistent performance (R²: {best_model_stats['CV_R2_Mean']:.3f} ± {best_model_stats['CV_R2_Std']:.3f})")
    ...
    for feature_idx in sig_features.index:
        if feature_idx != 'const':
            ...
            if feature_name != 'const':
                print(f"   • {feature_name}: {coef:.3f} {significance} (p={pval:.3f})")
...
VALIDATION APPROACH:
✅ Used entire dataset for maximum explanatory power (appropriate for explanatory modeling)
✅ Employed cross-validation for model stability assessment
✅ No train-test split needed as we're not doing predictive modeling
✅ This approach is statistically valid for understanding relationships in the data
""")
